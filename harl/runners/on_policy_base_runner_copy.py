...
from harl.utils.configs_tools_copy import init_dir, save_config
...
class OnPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args, agent_classes=None):
        ...
        Args:
            ...
            agent_classes: dict mapping class IDs to agent IDs.
        """
        self.args = args
        ...
        if self.share_param:
            ...
            for agent_id in range(self.num_agents):
                ...
                print(f'agent {agent_id} actor: {agent}')
                self.actor.append(agent)
        ...
        if self.use_class_ac:
            ...
            self.actor_buffer = {}
            self.critic_buffer = {}
            for key, value in self.agent_classes.items():
                if isinstance(key, str):  # Key is a class identifier string
                    class_id = value
                    agents = self.agent_classes[class_id]
                else:  # Key is an integer class ID
                    class_id = key
                    agents = value

                self.class_actors[class_id] = ALGO_REGISTRY[args["algo"]](
                    {**algo_args["model"], **algo_args["algo"]},
                    self.envs.observation_space[agents[0]],
                    self.envs.action_space[agents[0]],
                    device=self.device,
                )
                self.class_critics[class_id] = VCritic(
                    {**algo_args["model"], **algo_args["algo"]},
                ...
                self.actor_buffer[class_id] = OnPolicyActorBuffer(
                    {**algo_args["train"], **algo_args["model"]},
                    self.envs.observation_space[agents[0]],
                    self.envs.action_space[agents[0]],
                )
                if self.state_type == "EP":
                    self.critic_buffer[class_id] = OnPolicyCriticBufferEP(
                        {**algo_args["train"], **algo_args["model"], **algo_args["algo"]},
                        self.envs.share_observation_space[0],
                    )
                elif self.state_type == "FP":
                    self.critic_buffer[class_id] = OnPolicyCriticBufferFP(
                        {**algo_args["train"], **algo_args["model"], **algo_args["algo"]},
                        self.envs.share_observation_space[0],
                        len(agents),
                    )
    ...
    def run(self):
        ...
        print(f'entering training loop with {episodes} episodes')
        for episode in range(1, episodes + 1):
            if self.algo_args["train"][
                ...
                # Apply learning rate decay for class-specific critics
                if self.use_class_ac:
    ...
    def warmup(self):
        ...
        if self.use_class_ac:
            print(f"Actor buffer keys: {self.actor_buffer.keys()}")
            for class_id, agents in self.agent_classes.items():
                if isinstance(class_id, str):  # Skip the string keys
                    continue

                print(f"Processing class_id: {class_id}, agents: {agents}")
                for agent_id in agents:
                    print(f"Agent ID: {agent_id}")
                    self.actor_buffer[class_id].obs[0] = obs[:, agent_id].copy()
                    if self.actor_buffer[class_id].available_actions is not None:
                        self.actor_buffer[class_id].available_actions[0] = available_actions[:, agent_id].copy()

                if self.state_type == "EP":
                    self.critic_buffer[class_id].share_obs[0] = share_obs[:, 0].copy()
                elif self.state_type == "FP":
                    self.critic_buffer[class_id].share_obs[0] = share_obs.copy()
        else:
            for agent_id in range(self.num_agents):
    ...
    def collect(self, step):
        ...
        if self.use_class_ac:
            ...
            for class_id, agents in self.agent_classes.items():
                ...
                class_actions = []
                class_action_log_probs = []
                class_rnn_states = []

                for agent_id in agents:
                    action, action_log_prob, rnn_state = self.class_actors[class_id].get_actions(
                        self.actor_buffer[class_id].obs[step],
                        self.actor_buffer[class_id].rnn_states[step],
                        self.actor_buffer[class_id].masks[step],
                        self.actor_buffer[class_id].available_actions[step]
                        if self.actor_buffer[class_id].available_actions is not None
                        else None,
                    )
                    class_actions.append(_t2n(action))
                    class_action_log_probs.append(_t2n(action_log_prob))
                    class_rnn_states.append(_t2n(rnn_state))

                actions.append(np.array(class_actions).transpose(1, 0, 2))
                action_log_probs.append(np.array(class_action_log_probs).transpose(1, 0, 2))
                rnn_states.append(np.array(class_rnn_states).transpose(1, 0, 2, 3))

                if self.state_type == "EP":
                    value, rnn_state_critic = self.class_critics[class_id].get_values(
                        self.critic_buffer[class_id].share_obs[step],
                        self.critic_buffer[class_id].rnn_states_critic[step],
                        self.critic_buffer[class_id].masks[step],
                    )
                    values[class_id] = _t2n(value)
                    rnn_states_critic[class_id] = _t2n(rnn_state_critic)
                elif self.state_type == "FP":
                    value, rnn_state_critic = self.class_critics[class_id].get_values(
                        np.concatenate(self.critic_buffer[class_id].share_obs[step]),
                        np.concatenate(self.critic_buffer[class_id].rnn_states_critic[step]),
                        np.concatenate(self.critic_buffer[class_id].masks[step]),
                    )
    ...
    def insert(self, data):
        ...
        if self.use_class_ac:
            for class_id, agents in self.agent_classes.items():
                ...
                if self.state_type == "EP":
                    self.critic_buffer[class_id].insert(
                        share_obs[:, 0],
                        rnn_states_critic,
                        values,
                        rewards[:, 0],
    ...
    def compute(self):
        ...
        if self.use_class_ac:
            next_values = {}
            for class_id in self.class_critics:
                if self.state_type == "EP":
                    next_value, _ = self.class_critics[class_id].get_values(
                        self.critic_buffer[class_id].share_obs[-1],
                        self.critic_buffer[class_id].rnn_states_critic[-1],
                        self.critic_buffer[class_id].masks[-1],
                    )
                    next_values[class_id] = _t2n(next_value)
                elif self.state_type == "FP":
                    next_value, _ = self.class_critics[class_id].get_values(
                        np.concatenate(self.critic_buffer[class_id].share_obs[-1]),
                        np.concatenate(self.critic_buffer[class_id].rnn_states_critic[-1]),
                        np.concatenate(self.critic_buffer[class_id].masks[-1]),
                    )
                    next_values[class_id] = np.array(
                        np.split(_t2n(next_value), self.algo_args["train"]["n_rollout_threads"])
                    )
                self.critic_buffer[class_id].compute_returns(next_values[class_id], self.value_normalizer)
        else:
            if self.state_type == "EP":
    ...
    def after_update(self):
        ...
        if self.use_class_ac:
            for class_id, agents in self.agent_classes.items():
                ...
                self.critic_buffer[class_id].after_update()
        else:
            for agent_id in range(self.num_agents):
                self.actor_buffer[agent_id].after_update()
    ...
    def eval(self):
        ...
        while True:
            ...
            for agent_id in range(self.num_agents):
                class_id = self.agent_classes[agent_id]
                eval_actions, temp_rnn_state = self.class_actors[class_id].act(
                    eval_obs[:, agent_id],
    ...
    def save(self):
        ...
        if self.use_class_ac:
            for class_id, actor in self.class_actors.items():
                policy_actor = actor.actor
                torch.save(
                    policy_actor.state_dict(),
                    str(self.save_dir) + f"/actor_class{class_id}.pt",
                )
            for class_id, critic in self.class_critics.items():
                policy_critic = critic.critic
                torch.save(
                    policy_critic.state_dict(),
                    str(self.save_dir) + f"/critic_class{class_id}.pt",
                )
        else:
            for agent_id in range(self.num_agents):
        ...
        if self.use_class_ac:
            for class_id, actor in self.class_actors.items():
                policy_actor_state_dict = torch.load(
                    ...
                    + f"/actor_class{class_id}.pt"
                )
                actor.actor.load_state_dict(policy_actor_state_dict)
            for class_id, critic in self.class_critics.items():
                policy_critic_state_dict = torch.load(
                    str(self.algo_args["train"]["model_dir"])
                    + f"/critic_class{class_id}.pt"
                )
