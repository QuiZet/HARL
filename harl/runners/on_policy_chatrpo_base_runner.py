...
class OnPolicyCHATRPOBaseRunner:
    """Base runner for on-policy algorithms."""
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        self.class_ac = algo_args["algo"]["class_ac"]
        ...
        if self.class_ac:
            self.init_agent_classes()
            self.init_class_actors()
    ...
    def restore(self):
        ...
        if not self.algo_args["render"]["use_render"]:
            policy_critic_state_dict = torch.load(
                str(self.algo_args["train"]["model_dir"]) + "/critic" + ".pt"
            )
    ...
    def init_agent_classes(self):
        ...
        for agent_id in range(self.num_agents):
            obs_dim = self.envs.observation_space[agent_id].shape
            ...
            if class_key not in class_key_to_label or agent_id == 2:
                ...
                self.agent_classes[class_label] = {
                    f"class_{class_label}_number_of_agents": 0,
                    f"class_{class_label}_obs_action_dim": (obs_dim, action_dim),
                    f"class_{class_label}_agents": []
                }
    ...
    def init_class_actors(self):
        ...
        for class_label, class_info in self.agent_classes.items():
            ...
            agent = ALGO_REGISTRY[self.args["algo"]](
                ...
                self.envs.action_space[class_info[f"class_{class_label}_agents"][0]],
                device=self.device,
            )
            self.class_actors[class_label] = agent
    ...
    def get_class_label(self, agent_id):
        ...
        for class_label, class_info in self.agent_classes.items():
            if agent_id in class_info[f"class_{class_label}_agents"]:
                return class_label
        return None